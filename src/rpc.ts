...
export class TwirpRpc {
  ...
  async request(
    ...
    if (!response.ok) {
      ...
      try {
        if (isJson) {
          const parsedError = (await response.json()) as Record<
            string,
            unknown
          >;
        ...
        console.debug(
          `Error when trying to parse error message, using defaults`,
          e,
        );
      }
    ...
    const camelcaseKeys = await import("camelcase-keys").then(
      (mod) => mod.default,
    );
